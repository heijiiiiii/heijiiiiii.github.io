...
# 第一阶段：无模块化
JavaScript发展面临的问题：
随着AJAX技术的问世，前端开发者已经慢慢感受到前端逻辑的复杂性，JavaScript的代码量也在逐日增多。在项目较小的场景下，甚至只使用一个JavaScript文件将所有功能逻辑放在一起。但是随着项目体量的越来越大以及开源库的发展，开发者需要引入外部开源库时（如jQuery等库），就容易带来文件的依赖以及冲突。

解决方案：
简单的将所有需要用到的JavaScript文件统一引入到html文件中
...
* 当项目的JavaScript特别多的时候，需要手动维护JavaScript的加载先后循序，避免出现依赖出错
* 依赖关系不直观，不利于维护

思考：
开发者迫切的需要一种方案能解决全局作用域污染、维护JavaScript依赖关系。这个时候，用函数的形式实现一个模块化加载就出现了。
